...
def twofa_exists():
    return os.path.exists('C:\\Users\\vnaad\\OneDrive\\Desktop\\NSE\\config.txt')

# Function to retrieve the currently saved email
def get_saved_email():
    if twofa_exists():
        with open('C:\\Users\\vnaad\\OneDrive\\Desktop\\NSE\\config.txt', 'r') as f:
            return f.read().strip()
...
def add_gmail(gmail):
    try:
        if gmail:
            with open('C:\\Users\\vnaad\\OneDrive\\Desktop\\NSE\\config.txt', 'w') as f:
                f.write(gmail)
...
def remove_gmail():
    try:
        if twofa_exists():
            os.remove('C:\\Users\\vnaad\\OneDrive\\Desktop\\NSE\\config.txt')
...
def send_email(gmail, otp):
    sender_email = 'CORTEXOA@gmail.com'
