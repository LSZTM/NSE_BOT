...
def get_receiver_add():
    try:
        with open("C:\\Users\\vnaad\\OneDrive\\Desktop\\NSE\\config.txt") as f:
            email = f.read().strip()  
            if email:
                return email
...
def get_log_file():
    return "C:\\Users\\vnaad\\OneDrive\\Desktop\\NSE\\nse_report_downloader.log"
...
def send_mail(status, num_downloaded, num_validated, num_renamed):
    ...
    message["Subject"] = "Automation Run details"
    ...
    Number of Files Downloaded: {num_downloaded}
    Number of Files Validated: {num_validated}
